...
//
	...
	r2.xy = (r1 * r3.x + r1.zwzw); //mad r2.xy, r1, r3.x, r1.zwzw
	...
	float3 global_illumination = float3(1.0, 1.0, 1.0);
	if (k_is_lightmap_exist)
	{

		float3 unknown_texcoord0 = float3(v7.xy, 0.0625);
		float4 sample0 = tex3D(lightprobe_texture_array, unknown_texcoord0);


		float3 unknown_texcoord1 = float3(v7.xy, 0.1875);

		float4 sample1 = tex3D(lightprobe_texture_array, unknown_texcoord1);


		float3 color_aggregate = sample0.xyz + sample1.xyz;

		float3 alpha_aggregate = sample0.w * sample1.w;


		alpha_aggregate *= p_lightmap_compress_constant_0.x;
		...
		global_illumination = color_aggregate;
	}
	r1.xyz = global_illumination;
